...
import { doc, setDoc, collection, addDoc, query, orderBy, onSnapshot, where, or, and } from 'firebase/firestore';
...
function App() {
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (currentUser) => {
      setUser(currentUser);
      if (currentUser) {
        // Track users in the cloud database when they sign up/in
        await setDoc(doc(db, 'users', currentUser.uid), {
          uid: currentUser.uid,
          displayName: currentUser.displayName,
          email: currentUser.email,
          photoURL: currentUser.photoURL,
          searchName: currentUser.displayName.toLowerCase() // used for searching
        }, { merge: true });
      }
    });
    return () => unsubscribe();
  ...
  // 2. Search for other users in the cloud
  useEffect(() => {
  ...
  // 3. Listen for private messages between you and the active selected user
  useEffect(() => {
    ...
    // Generate a unique room ID combined of both user IDs ordered alphabetically
    ...
    return (
      <div style={styles.loginContainer}>
        <div style={styles.loginCard}>
          ...
          <p>Sign in with Google to search friends and start messaging.</p>
  ...
  return (
    <div style={styles.desktopApp}>
      ...
      <div style={styles.sidebar}>
        ...
        <div style={styles.userListContainer}>
          <div style={styles.sectionLabel}>
            {searchQuery ? 'Search Results' : 'Search for a username above'}
          </div>
      ...
      <div style={styles.chatWindow}>
        {activeChatUser ? (
          <>
            {/* Active Friend Header Bar */}
            <div style={styles.chatWindowHeader}>
              <img src={activeChatUser.photoURL} alt="" style={styles.avatar} />
              <div style={{ marginLeft: '12px', fontWeight: 'bold' }}>{activeChatUser.displayName}</div>
            </div>
          ...
          <div style={styles.emptyStateContainer}>
            ...
            <p>Use the sidebar search tool to look up a user and start a conversation.</p>
          </div>
        )}
      </div>
    </div>
...
// Full Desktop UI styling matrix
const styles = {
  loginContainer: { display: 'flex', height: '100vh', justifyContent: 'center', alignItems: 'center', backgroundColor: '#f0f2f5', fontFamily: 'sans-serif' },
  ...
  // Computer Desktop View Container
  desktopApp: { display: 'flex', width: '100vw', height: '100vh', backgroundColor: '#fff', fontFamily: 'sans-serif', overflow: 'hidden' },
  
  // Sidebar properties
  sidebar: { width: '350px', borderRight: '1px solid #e0e0e0', display: 'flex', flexDirection: 'column', backgroundColor: '#f7f8fa' },
  ...
  smallLogoutBtn: { padding: '4px 10px', backgroundColor: '#f44336', color: '#fff', border: 'none', borderRadius: '4px', fontSize: '12px', cursor: 'pointer' },
  searchBoxWrapper: { padding: '12px' },
  searchInput: { width: '100%', padding: '10px 12px', borderRadius: '8px', border: '1px solid #ccc', boxSizing: 'border-box', outline: 'none', fontSize: '14px' },
  ...
  userRowName: { marginLeft: '12px', fontWeight: '500', color: '#050505' },
  noUserHint: { padding: '15px', fontSize: '13px', color: '#888', textAlign: 'center' },

  // Main Chat frame
  chatWindow: { flex: 1, display: 'flex', flexDirection: 'column', backgroundColor: '#fff' },
  ...
  msgBubble: { padding: '10px 16px', borderRadius: '18px', maxWidth: '60%', fontSize: '15px', lineHeight: '1.4', boxShadow: '0 1px 1px rgba(0,0,0,0.05)' },
  messageInputForm: { display: 'flex', padding: '15px 20px', backgroundColor: '#fff', borderTop: '1px solid #e0e0e0', alignItems: 'center' },
  desktopInputField: { flex: 1, padding: '12px 16px', borderRadius: '24px', border: '1px solid #ccd0d5', outline: 'none', fontSize: '15px', backgroundColor: '#f0f2f5' },
  ...
  emptyStateContainer: { flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', color: '#8e9297', backgroundColor: '#f0f2f5' }
};
